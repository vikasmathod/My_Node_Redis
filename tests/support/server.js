      ...
      async.series({
        ...
        two: function (cb) {
          ...
          that.stdout_stream.once('open', function (fd) {
            ...
            that.server.stdout.pipe(that.stdout_stream, { end: false });
            that.server.stderr.pipe(that.stderr_stream, { end: false });
        ...
        three: function (cb) {
          ...
          if (that.tags === 'auth') {
            that.client = redis.createClient(that.port, that.host, { no_ready_check: true });
...
Server.prototype.kill_server = function (cpid, spid, k_callback) {
  ...
  try {
    ...
    setTimeout(function () { k_callback(null, true); }, 200);
  } catch (e) {
    k_callback(e, null);
  }
};
