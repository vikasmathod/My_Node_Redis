...
BgUtility.prototype.get_op = function (client, op, start, end, step, keyval, cb) {
  ...
  this.asyncFor(start, end, function (loop) {
    ...
    client[op](key, function (err, res) {
      ...
      if (val == res) loop.next();
      else cb(new Error("Value Not equal (" + val + "==" + res + ")"), null);
    });
  }, function () {
    cb(null, true);
  });
    ...
    client[op](keys[0], args[0], function (err, res0) {
      ...
      if (vals[0] == res0) {
        client[op](keys[1], args[1], function (err, res1) {
          ...
          else cb(new Error("Value Not equal (" + vals[1] + "==" + res1 + ")"), null);
        });
      }
      else cb(new Error("Value Not equal (" + vals[0] + "==" + res0 + ")"), null);
    });
  }, function () {
    cb(null, true);
  });
